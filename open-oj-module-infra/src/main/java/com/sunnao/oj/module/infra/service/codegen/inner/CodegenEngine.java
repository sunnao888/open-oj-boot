package com.sunnao.oj.module.infra.service.codegen.inner;
...
import com.sunnao.oj.framework.apilog.core.enums.OperateTypeEnum;
...
import com.sunnao.oj.framework.common.util.collection.CollectionUtils;
import com.sunnao.oj.framework.common.util.date.DateUtils;
import com.sunnao.oj.framework.common.util.date.LocalDateTimeUtils;
import com.sunnao.oj.framework.common.util.object.BeanUtils;
import com.sunnao.oj.framework.common.util.object.ObjectUtils;
...
import com.sunnao.oj.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.sunnao.oj.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.sunnao.oj.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.sunnao.oj.module.infra.enums.codegen.CodegenSceneEnum;
import com.sunnao.oj.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.sunnao.oj.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.sunnao.oj.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 open-oj-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 open-oj-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.sunnao.oj.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/open-oj-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/open-oj-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "open-oj-module-${table.moduleName}/" + // 顶级模块
                "open-oj-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "open-oj-module-${table.moduleName}/" + // 顶级模块
                "open-oj-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "open-oj-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "open-oj-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
