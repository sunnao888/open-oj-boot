package com.sunnao.oj.framework.websocket.core.sender.rabbitmq;
...
@RabbitListener(
        bindings = @QueueBinding(
                value = @Queue(
                        ...
                        name = "${open-oj.websocket.sender-rabbitmq.queue}" + "-" + "#{T(java.util.UUID).randomUUID()}",
                        // Consumer 关闭时，该队列就可以被自动删除了
                        autoDelete = "true"
                ),
                exchange = @Exchange(
                        name = "${open-oj.websocket.sender-rabbitmq.exchange}",
